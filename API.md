...
```
  ...
  ### 4. 更新 PV/UV 数据接口
 - **接口地址**：`POST /api/update-pv-uv/:eventId`
- **功能描述**：根据事件 ID 更新 PV 和 UV 数据。
 ### 请求参数
| 参数名 | 类型 | 是否必填 | 描述 |
| ---- | ---- | ---- | ---- |
| `eventId` | string | 是 | 事件ID，取值为 1、2 或 3（代表是子页面Page1或2，3的pv uv数据） |

#### 请求示例

``http://localhost:5501/api/update-pv-uv/1``
...
- **功能描述**：从 **InfluxDB** 数据库中查询最近 30（可以选择任意时间范围）天的页面平均停留时长数据。
#### 请求参数：无
...
| ---- | ---- | ---- |
| `success` | boolean | 请求是否成功 |
| `data` | array | 查询结果数组，包含页面路径和平均停留时长 |
#### 响应示例
```json
{
  "success": true,
